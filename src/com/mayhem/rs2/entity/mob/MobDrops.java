...
public class MobDrops {
	...
	public static void drop(Entity entity, Mob mob, ItemDropDefinition.ItemDropTable table, Location dropLocation) {
		if ((table != null) && (table.getDrops() != null)) {
			...
			if (entity.isNpc()) {
				if (item.getDefinition().getHighAlch() >= 1_000_000) {
	...
	public static void dropItems(Entity entity, Mob mob) {
		...
		case 100://rockcrab
			World.sendGlobalMessage("<col=4B0082><shad=0>The Rock Crab Has Been Slain by (" + player.getUsername() + " - Level:" + player.getSkill().getCombatLevel() + ")");
		...
		if ((entity != null) && (!entity.isNpc()) && ((p = World.getPlayers()[entity.getIndex()]) != null)) {
			...
			if (p.getController() == ControllerManager.RAIDS_CONTROLLER) {
				try {
					p.getRaids().handleMobDeath(mob);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
