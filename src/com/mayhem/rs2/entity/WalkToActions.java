...
import com.mayhem.rs2.content.YanChest;
...
public class WalkToActions {
	...
	public static void clickObject(final Player player, final int option, final int id, final int x, final int y) {
		...
		case 29241:
			...
			player.getClient().queueOutgoingPacket(new SendMessage("Your Hitpoints, Prayer, Run, and Special have been restored."));
	...
	public static final void finishClickNpc(Player player, int option, Mob mob) {
		...
		case 1:// NPC FIRST CLICK
			...
			case 7690://Infernal Tzhaar Kek
				player.start(new OptionDialogue("Enter the Inferno", p -> {
					...
					player.getInferno().gamble(player);
			...
			case 732:// Farming Store
				...
				break;
				case 7663:
				player.start(new OptionDialogue("Obtain a task.", p -> {
					if (player.getSlayer().hasTask()) {
						DialogueManager.sendNpcChat(player, 7663, Emotion.ANGRY_1, "You already have a task!", "Complete your current task or please reset it...");
						return;
					}
					player.getSlayer().assign(Slayer.SlayerDifficulty.WILDERNESS);
					DialogueManager.sendNpcChat(player, 7663, Emotion.HAPPY, "You have to kill "+player.getSlayer().getAmount()+" "+player.getSlayer().getTask() + "'s." );
				} , "View Rewards", p -> {
					player.getShopping().open(6);
				},"Reset my task (@red@250k@bla@)", p -> {
					if (!player.getSlayer().hasTask()) {
						DialogueManager.sendNpcChat(player, 7663, Emotion.ANGRY_1, "You do not even have a slayer task!");
						return;
					} else if (!player.getInventory().hasItemAmount(995, 250000)) {
						DialogueManager.sendNpcChat(player, 7663, Emotion.ANGRY_1, "You do not even have 250,000 coins!");
						return;
					} else {
		...
		case 2:// NPC SECOND CLICK
			...
			case 5787:// Agility
				...
				break;
			case 7042:// Swagster
				player.start(new OptionDialogue("SkyScapeLive Credits Store 1", p -> {
			...
			case 306:// tutorial npc
				...
				} , "Server tour(Show several areas around SkyScapeLive)", p -> {
	...
	public static void finishObjectClick(Player player, int id, int option, int x, int y) {
		...
		if(player.getRaids().handleObjectClick(player, id)) {
			return;
		}
		...
		case 1:
			...
			if (id == 8720 || id == 26820 || id == 26813) {
				player.send(new SendString("http://www.SkyScapeLive.everythingrs.com/services/vote", 12000));
			...
			switch (id) {   //Object Options
			
			//Yanille Stairs Gen Shop
			case 16683: // UP
				player.teleport(new Location(2597, 3106, 1));
				player.send(new SendMessage("You climb up the stairs to the next floor."));
				break;
			case 16681: // Down
				player.teleport(new Location(2597, 3106, 0));
			...
			case 2191:
				...
				break;
			case 375:
				if (player.getSkill().getLevels()[17] >= 70) {
					YanChest.searchChest(player, x, y);
				} else {
					player.send(new SendMessage("You need a level of at least 70 thieving to steal from this chest."));
				}
				break;
			case 27282:
