...
public class ItemPackets extends IncomingPacket {
	...
	public void handle(Player player, StreamBuffer.InBuffer in, int opcode, int length) {
		...
		case 53: // ITEM ON ITEM
			...
			if (usedWith.getId() == 19550 || usedWith.getId() == 20655 && itemUsed.getId() == 2550
					|| itemUsed.getId() == 2551) {
		...
		case 122: // FIRST CLICK
			...
			case 21034: //rigour
				player.start(new OptionDialogue("Unlock Rigour", p -> {
					player.setRigour(1);
					player.getInventory().remove(21034, 1);
					player.sendMessage("You have unlocked rigour.");
					player.send(new SendRemoveInterfaces());
				} , "Cancel", p -> {
					player.send(new SendRemoveInterfaces());
				}));	
			break;
			case 21079: //augury
				player.start(new OptionDialogue("Unlock Augury", p -> {
					player.setAugury(1);
					player.getInventory().remove(21079, 1);
					player.sendMessage("You have unlocked augury.");
					player.send(new SendRemoveInterfaces());
				} , "Cancel", p -> {
					player.send(new SendRemoveInterfaces());
				}));	
