...
import com.mayhem.rs2.content.minigames.raids.Raids;
...
public class Player extends Entity {
	...
	public Raids raids = new Raids(this);
	...
	public void setRigour(int unlocked) {
		rigour = unlocked;
	}
	
	public void setAugury(int unlocked) {
		augury = unlocked;
	}
...
private MysterySpin donorBox= new MysterySpin(this);
	...
	public long getyanchest() {
		return yanchestTimer;
	}
	...
	public boolean login(boolean starter) throws Exception {
		...
		send(new SendString("</col>SkyScapeLive Credits: @gre@" + Utility.format(this.getCredits()), 52504));
		...
		//send(new SendMessage("<img=7>@blu@Welcome to SkyScapeLive!"));
		send(new SendMessage("@blu@Welcome to SkyScapeLive!"));
		send(new SendMessage("@blu@Server is currently in Alpha/Beta Mode"));
		send(new SendMessage("@blu@Everything you do will not carry over into the main game. More information to follow."));
		send(new SendMessage("@blu@XP Rates x1000 to not waste your time during this testing stage!"));
		send(new SendMessage("@blu@Make sure to keep active on discord, throw your suggestions at steven."));
		
		
		if (World.getActivePlayers() > Constants.MOST_ONLINE) {
			Constants.MOST_ONLINE = World.getActivePlayers();
			FileHandler.saveMaxPlayers();
			World.sendGlobalMessage("[<col=910D0D>SkyScapeLive</col>] We have broken our most players online record! New record: <col=910D0D>" + Constants.MOST_ONLINE + "</col>!");
		}
		
		if (Constants.doubleExperience) {			
			send(new SendMessage("<img=7>@dre@[EVENT] DOUBLE EXPERIENCE WEEKEND IS:</col> @gre@[ACTIVE]"));
		}
	...
	public String determineRank(Player player) {
		...
		case 5:
			return "<col=4D8528>Normal Donator</col>";//red
		case 6:
