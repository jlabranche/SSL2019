	...
	public static void plantFlower(Player player) {
		...
		TaskQueue.queue(new Task(player, 1, false) {
			...
			public void execute() {
				walk(player);
				stop();
				ObjectManager.removeActive(ObjectManager.getGameObject(loc.getX(), loc.getY(), loc.getZ()));
			}
		...
		TaskQueue.queue(new Task(player, 10, false) {

			@Override
			public void execute() {
				ObjectManager.deleteWithObject(loc.getX(), loc.getY(), loc.getZ());
				player.sendMessage("Remove plant");
