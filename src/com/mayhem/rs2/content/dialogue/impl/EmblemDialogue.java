...
public class EmblemDialogue extends Dialogue {
	 ...
	 * The tier data
	 */
	public int[][] TIER_DATA = { { 12746, 25_000 }, { 12748, 50_000 }, { 12749, 150_000 }, { 12750, 225_000 }, { 12751, 500_000 }, { 12752, 750_000 }, { 12753, 1_250_000 }, { 12754, 2_000_000 }, { 12755, 3_000_000 }, { 12756, 5_000_000 } };
	...
	public boolean clickButton(int id) {
		...
		case DialogueConstants.OPTIONS_5_1:
			for (int i = 0; i < TIER_DATA.length; i++) {
				if (player.getInventory().hasItemId(new Item(TIER_DATA[i][0]))) {
					...
					bountyTotal = 0;
					player.getInventory().remove(new Item(TIER_DATA[i][0],player.getInventory().getItemAmount(TIER_DATA[i][0])));
					for (int[] emblem : TIER_DATA) {
						player.getInventory().remove(new Item(emblem[0],player.getInventory().getItemAmount(emblem[0])));
					}
					break;
				} else {
		...
		case DialogueConstants.OPTIONS_5_5:
			if (player.getCredits() < 3) {
				DialogueManager.sendStatement(player, "You need 3 SkyScapeLive Credits to do this!");
				return false;
			}
