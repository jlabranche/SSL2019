...
/**
	...
	public void gamble(Player player) {
		...
		if (!player.getInventory().hasItemId(INFERNAL_CAPE)) {
			player.send(new SendMessage("You do not have a Infernal cape in your inventory."));
			return;
		}
		player.getInventory().remove(INFERNAL_CAPE, 1);

		if (Utility.random(100) >= 95) { //5 Percent chance
			int random = Utility.random(3);
			switch(random) {
				case 0:
				World.sendGlobalMessage("[@red@GAMBLE@bla@] " + player.getUsername()
						+ " received a pet from Inferno.");
				player.getInventory().addOrCreateGroundItem(22319, 1, true);
				break;
				case 1:
					player.getInventory().addOrCreateGroundItem(ENRAGED_CAPE, 1, true);
					player.sendMessage("@red@Congratulations, you received a upgraded version of the Infernal Cape.");
				break;
				case 2:
				...
				case 3:
					World.sendGlobalMessage("[@red@GAMBLE@bla@] " + player.getUsername()
							+ " received a pet from Inferno.");
					player.getInventory().addOrCreateGroundItem(21291, 1, true);
				break;
