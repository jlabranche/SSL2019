...
public class Combat {
	...
	public boolean isWithinDistance(int req) {
		if (!entity.isNpc() && !attacking.isNpc()
				&& Utility.getManhattanDistance(attacking.getLocation(), entity.getLocation()) == 0) {
			return false;
		}
		...
		if (entity.isNpc()) {
			...
			if (m != null) {
				if (m.getId() == 8596) {
					dist = 18;
					ignoreClipping = true;
				} else if (m.getId() == 7691) {
					dist = 3;
					ignoreClipping = true;
				} else if (m.getId() == 7706) {
					dist = 15;
					ignoreClipping = true;
		...
		if (!entity.isNpc()) {
			if (type == CombatTypes.MELEE) {
				if (attacking.isNpc()) {
					...
					if (m != null) {
						if (m.getId() == 3847) {
							dist = 2;
							ignoreClipping = true;
						}else if (m.getId() == 7553 || m.getId() == 7554 || m.getId() == 7555) {
							dist = 4;
							ignoreClipping = true;
						} else if (m.getId() == 2042 || m.getId() == 2043 || m.getId() == 2044) {
							dist = 25;
				...
				if (attacking.isNpc()) {
					Mob m = World.getNpcs()[attacking.getIndex()];
					if (m != null) {
						if (m.getId() == 2042 || m.getId() == 7706 || m.getId() == 2043 || m.getId() == 2044) {
							dist = 25;
							ignoreClipping = true;
						}else if (m.getId() == 7553 || m.getId() == 7554 || m.getId() == 7555) {
							dist = 20;
