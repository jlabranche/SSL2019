...
public class PrayerBook {
	...
	private static final Prayer[] OVER_HEAD_DISABLED = new Prayer[] { Prayer.PROTECT_FROM_MAGIC,
			Prayer.PROTECT_FROM_RANGE, Prayer.PROTECT_FROM_MELEE, Prayer.RETRIBUTION, Prayer.REDEMPTION, Prayer.SMITE };
	private static final Prayer[] DEFENCE_DISABLED = new Prayer[] { Prayer.THICK_SKIN, Prayer.ROCK_SKIN,
			Prayer.STEEL_SKIN, Prayer.CHIVALRY, Prayer.PIETY };
	private static final Prayer[] ATTACK_DISABLED = new Prayer[] { Prayer.CLARITY_OF_THOUGHT, Prayer.IMPROVED_REFLEXES,
			Prayer.INCREDIBLE_REFLEXES, Prayer.SHARP_EYE, Prayer.HAWK_EYE, Prayer.EAGLE_EYE, Prayer.MYSTIC_WILL,
			Prayer.MYSTIC_LORE, Prayer.MYSTIC_MIGHT, Prayer.CHIVALRY, Prayer.PIETY };
	private static final Prayer[] STRENGTH_DISABLED = new Prayer[] { Prayer.BURST_OF_STRENGTH,
			Prayer.SUPERHUMAN_STRENGTH, Prayer.ULTIMATE_STRENGTH, Prayer.SHARP_EYE, Prayer.HAWK_EYE, Prayer.EAGLE_EYE,
			Prayer.MYSTIC_WILL, Prayer.MYSTIC_LORE, Prayer.MYSTIC_MIGHT, Prayer.CHIVALRY, Prayer.PIETY };
	private static final Prayer[] ATT_STR_DISABLED = new Prayer[] { Prayer.CLARITY_OF_THOUGHT, Prayer.IMPROVED_REFLEXES,
			Prayer.INCREDIBLE_REFLEXES, Prayer.BURST_OF_STRENGTH, Prayer.SUPERHUMAN_STRENGTH, Prayer.ULTIMATE_STRENGTH,
	...
	private static final Prayer[] COMBAT_DISABLED = new Prayer[] { Prayer.CLARITY_OF_THOUGHT, Prayer.IMPROVED_REFLEXES,
			Prayer.INCREDIBLE_REFLEXES, Prayer.BURST_OF_STRENGTH, Prayer.SUPERHUMAN_STRENGTH, Prayer.ULTIMATE_STRENGTH,
			Prayer.THICK_SKIN, Prayer.ROCK_SKIN, Prayer.STEEL_SKIN, Prayer.SHARP_EYE, Prayer.HAWK_EYE, Prayer.EAGLE_EYE,
			Prayer.MYSTIC_WILL, Prayer.MYSTIC_LORE, Prayer.MYSTIC_MIGHT, Prayer.CHIVALRY, Prayer.PIETY, Prayer.RIGOUR,
			Prayer.AUGURY };
	...
	public enum PrayerType {
		OVER_HEAD, DEFENCE, ATTACK, STRENGTH, MAGE_RANGE, COMBAT, DEFAULT
	}

	public enum Prayer {
		THICK_SKIN("Thick Skin", 1, 12.0, 83, PrayerType.DEFENCE), 
		BURST_OF_STRENGTH("Burst of Strength", 4, 12.0, 84,
				PrayerType.STRENGTH), 
		CLARITY_OF_THOUGHT("Clarity of Thought", 7, 12.0, 85,
						PrayerType.ATTACK), 
		...
		IMPROVED_REFLEXES("Improved Reflexes", 16, 8.0, 88, PrayerType.ATTACK), RAPID_RESTORE("Rapid Restore", 19, 60.0,
																89, PrayerType.DEFAULT), RAPID_HEAL("Rapid Heal", 22,
																		60.0, 90, PrayerType.DEFAULT), PROTECT_ITEM(
																				"Protect Item", 25, 30.0, 91,
																				PrayerType.DEFAULT), HAWK_EYE(
																						"Hawk Eye", 26, 6.0, 702,
																						PrayerType.MAGE_RANGE), MYSTIC_LORE(
																								"Mystic Lore", 27, 6.0,
																								703,
																								PrayerType.MAGE_RANGE), STEEL_SKIN(
																										"Steel Skin",
																										28, 6.0, 92,
																										PrayerType.DEFENCE), ULTIMATE_STRENGTH(
																												"Ultimate Strength",
																												31, 6.0,
																												93,
																												PrayerType.STRENGTH), INCREDIBLE_REFLEXES(
																														"Incredible Reflexes",
																														34,
																														6.0,
																														94,
																														PrayerType.ATTACK), PROTECT_FROM_MAGIC(
																																"Protect from Magic",
																																37,
																																4.0,
																																95,
																																PrayerType.OVER_HEAD), PROTECT_FROM_RANGE(
																																		"Protect from Range",
																																		40,
																																		4.0,
																																		96,
																																		PrayerType.OVER_HEAD), PROTECT_FROM_MELEE(
																																				"Protect from Melee",
																																				43,
																																				4.0,
																																				97,
																																				PrayerType.OVER_HEAD), EAGLE_EYE(
																																						"Eagle Eye",
																																						44,
																																						6.0,
																																						704,
																																						PrayerType.MAGE_RANGE), MYSTIC_MIGHT(
																																								"Mystic Might",
																																								45,
																																								6.0,
																																								705,
																																								PrayerType.MAGE_RANGE), RETRIBUTION(
																																										"Retribution",
																																										46,
																																										4.0,
																																										98,
																																										PrayerType.OVER_HEAD), REDEMPTION(
																																												"Redemption",
																																												49,
																																												3.0,
																																												99,
																																												PrayerType.OVER_HEAD), SMITE(
																																														"Smite",
																																														52,
																																														4.0,
																																														100,
																																														PrayerType.OVER_HEAD), CHIVALRY(
																																																"Chivalry",
																																																60,
																																																3.0,
																																																706,
																																																PrayerType.COMBAT), PIETY(
																																																		"Piety",
																																																		70,
																																																		3.0,
																																																		707,
																																																		PrayerType.COMBAT), RIGOUR(
																																																				"Rigour",
																																																				73,
																																																				3.0,
																																																				710,
																																																				PrayerType.COMBAT), AUGURY(
																																																						"Augury",
																																																						77,
																																																						3.0,
																																																						712,
	...
	private boolean canToggle(Prayer prayer) {
		if (player.getMaxLevels()[5] < prayer.getLevel()) {
			player.send(new SendMessage(
					"You need a Prayer level of " + prayer.getLevel() + " to use " + prayer.getName() + "."));
			return false;
			...
			return false;
		} else if (prayer == Prayer.RIGOUR) {// || (player.getMaxLevels()[5] < 65
			if (player.getRigour() == 0) {
				player.send(new SendMessage("You need to have unlocked rigour before you can use it."));
				player.send(new SendConfig(711, 1));
				return false;
			} else if (player.getMaxLevels()[5] < 73) {
				player.send(new SendMessage("You need a prayer level of 73 to use rigour."));
				player.send(new SendConfig(711, 1));
				return false;
			}
		} else if (prayer == Prayer.AUGURY && player.getAugury() == 0) {
			if (player.getAugury() == 0) {
				player.send(new SendMessage("You need to have unlocked augury before you can use it."));
				player.send(new SendConfig(713, 1));
				return false;
			} else if (player.getMaxLevels()[5] < 77) {
				player.send(new SendMessage("You need a prayer level of 77 to use augury."));
				player.send(new SendConfig(713, 1));
				return false;
			}
	...
	public boolean clickButton(int button) {
		...
		case 77115:
			...
			return true;
		case 175200:
			toggle(Prayer.RIGOUR);
			return true;
		case 175204:
			toggle(Prayer.AUGURY);
			return true;
		}
			...
			if (active(Prayer.PROTECT_FROM_MELEE)) {
				...
				if (target != null) {
					if (target.isNpc()) {
						...
						if ((id == 7388) || (id == 7390) || (id == 7401) || (id == 7409) || (id == 7410)
								|| (id == 7411)) {
							return hit.getDamage() / 2;
						}
						if (id == 1540) {
						...
						if ((id != 1677) && (id != 8133))
							return Utility.random(100) >= 65 ? 0 : hit.getDamage()/6;
					} else {
